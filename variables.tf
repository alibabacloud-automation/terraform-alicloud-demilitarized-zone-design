...
variable "vpcs" {
  ...
  validation {
    condition     = length(var.vpcs) == 3
    error_message = "The length of vpcs must be three."
  }

  validation {
    condition     = contains(keys(var.vpcs), "dmz") && contains(keys(var.vpcs), "dev") && contains(keys(var.vpcs), "prd")
    error_message = "The keys must be dmz, dev and prd."
  }
}
...
variable "nlb_server_group_health_check" {
  ...
  validation {
    condition     = length(var.nlb_server_group_health_check) == 1
    error_message = "The max length of health_check list of nlb_server_group is one."
  }
}
